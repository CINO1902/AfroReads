...
class UserDatasourceimp implements UserDataSource {
  ...
  @override
  Future<List> ChangeRestrictionAge(RestrictMode restric) async {
    List returnvalue = [];

    final response = await httpService.request(
        url: '/set_restriction_mode',
        methodrequest: RequestMethod.post,
        data: restrictModeToJson(restric));
    if (response.data['status'] == '1') {
      final errorvalue = ['1'];
  ...
  @override
  Future<List> ChangeUnsuitableGenres(RestrictMode restric) async {
    List returnvalue = [];

    final response = await httpService.request(
        url: '/set_unsuitable_genres',
        methodrequest: RequestMethod.post,
        data: restrictModeToJson(restric));
    if (response.data['status'] == '1') {
      final errorvalue = ['1'];
  ...
  Future<List> changeCustomTime(TimeModel time) async {
    ...
    final response = await httpService.request(
        url: '/change_time_limit',
        methodrequest: RequestMethod.post,
        data: timeModelToJson(time));
    if (response.data['status'] == '1') {
      final errorvalue = ['1'];
